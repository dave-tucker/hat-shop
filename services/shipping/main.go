...
func (s *server) consumeOrders(ctx context.Context) {
	r := kafka.NewReader(kafka.ReaderConfig{
		...
		GroupID:     "shipping",
		// StartOffset controls where a consumer group starts when it has no
		// committed offset (fresh Kafka, wiped volume, new deployment).
		// Default is LastOffset (high-watermark) which silently skips any
		// messages already in the topic at consumer startup — exactly the
		// bug we hit. FirstOffset means: start from the beginning and let
		// ON CONFLICT DO NOTHING in createShipment() handle idempotency.
	...
	for {
		...
		slog.Info("shipment preparing", "order_id", event.OrderID, "delay", shippingDelay())
